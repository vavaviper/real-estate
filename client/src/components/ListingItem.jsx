...
export default function ListingItem({ listing }) {
  return (
    <div className='card-soft w-full sm:w-[330px]'>
      <Link to={`/listing/${listing._id}`}>
        ...
        <div className='p-3 flex flex-col gap-2 w-full'>
          ...
          <div className='flex items-center gap-1'>
            <MdLocationOn className='h-4 w-4 text-primary-600' />
            <p className='text-sm text-gray-600 truncate w-full'>
              {listing.address}
            </p>
          </div>
          <p className='text-sm text-gray-600 max-h-[3.4rem] overflow-hidden'>
            {listing.description}
          </p>
          <p className='mt-2'>
